...
df_means_for_prediction['rolling_mean_t30'] = \
    df_means_for_prediction.groupby(['id'])['sales'].transform(lambda x: x.rolling(30).mean())
df_means_for_prediction['rolling_mean_t7'] = \
    df_means_for_prediction.groupby(['id'])['sales'].transform(lambda x: x.rolling(7).mean())
df_means_for_prediction['rolling_mean_t14'] = \
    df_means_for_prediction.groupby(['id'])['sales'].transform(lambda x: x.rolling(14).mean())
for i in range(7):
    df_means_for_prediction[f'prev_{i}'] = \
        df_means_for_prediction.groupby(['id'])['sales'].transform(lambda x: x.shift(i))
for i in range(4):
...
for i in range(4):
    df_means_for_prediction[f'event_name_1_prev_{i+1}'] = \
        df_means_for_prediction.groupby(['id'])['event_name_1'].transform(lambda x: x.shift(i))
...
for k in range(28 * 2):
    ...
    df_for_train['rolling_mean_t30'] = \
        df_for_train.groupby(['id'])['sales'].transform(lambda x: x.shift(k+1).rolling(30).mean())
    df_for_train['rolling_mean_t7'] = \
        df_for_train.groupby(['id'])['sales'].transform(lambda x: x.shift(k+1).rolling(7).mean())
    df_for_train['rolling_mean_t14'] = \
        df_for_train.groupby(['id'])['sales'].transform(lambda x: x.shift(k+1).rolling(14).mean())
    for i in range(7):
        df_for_train[f'prev_{i}'] = \
            df_for_train.groupby(['id'])['sales'].transform(lambda x: x.shift(k+i+1))
    for i in range(4):
        df_for_train[f'prev_week_{i}'] = \
            df_for_train.groupby(['id'])['sales'].transform(lambda x: x.shift(k + i * 7 + 1).rolling(7).mean())
    for i in range(1, 5):
        df_for_train[f'event_name_1_prev_{i}'] = \
            df_for_train.groupby(['id'])['event_name_1'].transform(lambda x: x.shift(k + i))
    for i in range(1, 4):
        df_for_train[f'event_name_1_prev_{i}_to_predict'] = \
            df_for_train.groupby(['id'])['event_name_1'].transform(lambda x: x.shift(i))
    for i in range(1, 4):
        df_for_train[f'event_name_1_next_{i}_to_predict'] = \
            df_for_train.groupby(['id'])['event_name_1'].transform(lambda x: x.shift(-i))
    ...
    columns = ['wday', 'month', 'year', 'event_name_1',
           'rolling_mean_t30', 'rolling_mean_t7', 'rolling_mean_t14'] + \
              [f'prev_{i}' for i in range(7)] + \
                [f'prev_week_{i}' for i in range(4)] + \
                [f'event_name_1_prev_{i}' for i in range(1, 5)] + \
              [f'event_name_1_prev_{i}_to_predict' for i in range(1, 4)] + \
                [f'event_name_1_next_{i}_to_predict' for i in range(1, 4)]
    ...
    for i in range(1, 4):
        df_for_prediction[f'event_name_1_prev_{i}_to_predict'] = \
            df_calendar.loc[len(df_calendar) - 28 * 2 + k - i, 'event_name_1']

    for i in range(1, 4):
        if k + i < 28 * 2:
            df_for_prediction[f'event_name_1_next_{i}_to_predict'] = \
                df_calendar.loc[len(df_calendar) - 28 * 2 + k + i, 'event_name_1']
        else:
            df_for_prediction[f'event_name_1_next_{i}_to_predict'] = event_encoder.transform(['unknown'])[0]
...
df_out.to_csv("output_lgbm_04.csv", index=False)
